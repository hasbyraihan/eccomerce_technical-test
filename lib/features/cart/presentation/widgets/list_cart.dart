          ...
          children: [
            ...
            ClipRRect(
              ...
              child: Image.network(
                ...
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) => const Icon(Icons.broken_image, size: 90),
              ),
            ),
                    ...
                    children: [
                      ...
                      InkWell(
                        borderRadius: BorderRadius.circular(
                          30,
                        ), // Agar efek ripple bulat  
                        onTap: () async {
                          // 1. Jadikan fungsi ini 'async'
                          // 2. Tampilkan dialog konfirmasi dan tunggu hasilnya (await)
                          final bool? confirmed = await showAppDialog(
                            context: context,
                            type: DialogType.confirmation,
                            title: "Remove Item",
                            message:
                                "Are you sure to remove ${cartItem.title} from Cart?",
                            confirmButtonText: "Remove",
                            cancelButtonText: "Cancel",
                          );

                          // 3. Cek apakah pengguna menekan tombol "Ya, Hapus" (dialog mengembalikan true)
                          if (confirmed == true) {
                            ...
                            // 5. Tampilkan dialog sukses. Tidak perlu 'await' jika tidak ada aksi setelahnya.
                            // Pastikan context masih valid sebelum memanggil dialog kedua.
                            if (context.mounted) {
                              showAppDialog(
                                context: context,
                                type: DialogType.success,
                                title: "Success",
                                message: "${cartItem.title} has been removed.",
                              );
                            }
                          }
                        },
